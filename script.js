...
function openModal(playerId) {
  ...
  const tierItems = tierOrder
    .filter(t => player.tiers[t])
    .map(t => `
      <div class="modal-tier-item">
        <span class="modal-tier-label">${gamemodeLabels[t] || t}:</span>
        <span class="modal-tier-value tier-badge ${t}">${player.tiers[t]}</span>
      </div>
    `).join('');
  const missingGamemodes = Object.keys(gamemodeLabels).filter(g => g !== 'overall' && !player.tiers[g]);
  if (missingGamemodes.length) {
    missingGamemodes.forEach(g => {
      tierItems += `
        <div class="modal-tier-item">
          <span class="modal-tier-label">${gamemodeLabels[g]}:</span>
          <span class="modal-tier-value" style="color:var(--text-muted)">N/A</span>
        </div>
      `;
    });
  }

  const socYt = player.socials.youtube
    ? `<a href="${player.socials.youtube}" target="_blank" class="modal-social-link youtube"><i class="fab fa-youtube"></i> YouTube</a>`
    : `<span class="modal-social-link youtube disabled"><i class="fab fa-youtube"></i> YouTube</span>';

  const socDisc = player.socials.discord
    ? `<a href="${player.socials.discord}" target="_blank" class="modal-social-link discord"><i class="fab fa-discord"></i> Discord</a>`
    : `<span class="modal-social-link discord disabled"><i class="fab fa-discord"></i> Discord</span>`;

  const socTwt = player.socials.twitter
    ? `<a href="${player.socials.twitter}" target="_blank" class="modal-social-link twitter"><i class="fab fa-twitter"></i> Twitter</a>`
    : `<span class="modal-social-link twitter disabled"><i class="fab fa-twitter"></i> Twitter</span>`;
